...
@section('content')
    ...
    <div style="margin-top: 100px">
                ...
                <div class="col-4">
                    ...
                    @foreach ($user->followers as $follower)
                        <div class="row align-items-center mt-3">
                            ...
                            <div class="col-auto ">
                                {{-- Show a Follow(store) or Following (destroy) btn. Do not show anything if its the Auth user. --}}
                                @if (Auth::user()->id != $follower->follower->id)
                                    ...
                                    @if ($follower->follower->isFollowed())
                                        <form action="{{ route('follow.destroy',$follower->follower->id)}}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-outline-secondary border-0 bg-transparent p-0 text-secondary">Following</button>
                                        </form>
                                    @else
                                        <form action="{{ route('follow.store',$follower->follower->id)}}" method="post">
                                            @csrf
                                            <button type="submit" class="btn btn-outline-secondary border-0 bg-transparent p-0 text-primary">Follow</button>
                                        </form>
                                    @endif
        ...
        @else
            <h3 class="text-secondary text-center">No Followers Yet</h3>
        @endif
    </div>
@endsection